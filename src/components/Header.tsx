...
export default function Header({ onGoToQuiz }: HeaderProps) {
  ...
  return (
    <header
      ...
      style={{
        background:
          "linear-gradient(135deg, #1a2d47 0%, #2d4a6b 35%, #3d3a2e 100%)",
      }}
    >
      {/* Faixa inferior */}
      <div
        ...
        style={{
          background:
            "linear-gradient(90deg, transparent, #E96C6C, #F8B6B0, transparent)",
        }}
      />
      ...
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at right, #FAD0CB 0%, transparent 70%)",
        }}
      />
      <div
        className="absolute top-0 left-0 w-24 h-full opacity-10 pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse at left, #FAD0CB 0%, transparent 70%)",
        }}
      />
        ...
        <nav className="hidden md:flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <a
              ...
              href={link.href}
              onClick={(e) =>
                handleNavClick(e, link.href, link.isQuiz)
              }
      ...
      {menuOpen && (
        <nav
          className="md:hidden absolute top-full left-0 right-0 py-4 px-4 shadow-lg z-20"
          style={{
            background:
              "linear-gradient(135deg, #2d4a6b 0%, #1a2d47 50%, #3d3a2e 100%)",
          }}
        >
